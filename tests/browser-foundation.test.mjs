...
test('browser UI keeps direct file timestamping local and pending semantics explicit', () => {
  ...
  assert.match(index, /Anchors to Bitcoin/)
  assert.match(index, /Bitcoin anchoring is still pending/)
  assert.match(index, /check again in about 3 hours/i)
...
test('create flow uses plain wording and visible timestamp progress', () => {
  ...
  assert.match(index, />Timestamp file</)
  assert.match(index, /id="timestamp-action-status"[^>]*role="status"/)
  assert.match(index, /Only a blinded fingerprint is sent to OpenTimestamps\. Your file stays on this device/)
  assert.match(index, /Timestamp proof created/)
  assert.match(app, /Creating timestamp…/)
  assert.match(app, /Creating your OpenTimestamps proof\. This can take a few seconds\./)
  assert.match(app, /setTimestampActionStatus/)
